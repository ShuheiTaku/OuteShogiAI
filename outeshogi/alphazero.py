...
class ZeroParameters:
    ...
    -------------------------------------------------------------
    N.key:盤面のハッシュ N.value:その盤面の訪問回数
    W.key:盤面のハッシュ W.value:その盤面の累積評価値
    P.key:盤面のハッシュ P.value:その盤面における着手ごとの確率
    '''
    def __init__(self):
        self.N = {}
        self.W = {}
...
class AlphaZero:
    ...
    def infer(self, x):
        ...
        policy = policy.cpu()
        value = value.cpu()
        policy = policy[0].numpy() # (1, 2172)shapeの二次元配列なので一次元にする
        value = torch.sigmoid(value).item()  # シグモイドをかけて勝率に変換する
        return policy, value  
    ...
    def make_legalmove_probs(self, board, policy):
        ...
        max_prob = max(legal_move_probs[legal_move_probs != 0])  # 確率が負の値であってはいけないので0-1正規化する
